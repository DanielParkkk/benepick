...
if __name__ == "__main__":
    test_queries = [
        ("서울 청년 월세 지원 받을 수 있어요?", "ko", {"age": 27, "region": "서울", "income_level": "중위소득 52%"}),
        ("취업 준비생 훈련비 지원", "ko", None),
        ("노인 돌봄 서비스", "ko", None),
        ("기초생활수급자 혜택 뭐가 있어요?", "ko", None),
        ("disability support benefits", "en", None),
    ]

    for query, lang, condition in test_queries:
        result = benepick_rag(query, lang_code=lang, user_condition=condition)
        if result["success"]:
            data = result["data"]
            print(f"✅ 답변: {data['answer'][:100]}...")
            print(f"   검색 시간: {data['search_time_ms']}ms | 문서 수: {data['doc_count']}개")
            print(f"   Top 정책: {data['docs_used'][0]['policy_name']} (score: {data['docs_used'][0]['score']})")
        else:
