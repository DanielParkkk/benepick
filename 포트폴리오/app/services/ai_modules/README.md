...
## 이번 수정 포인트
### 0) 혜택 금액 구조화 추가
- `benefit_extractor.py`: 정책 원문에서 금액, 기간, 계산 방식을 정규식/룰 기반으로 추출
- 요약 결과에 `unit_amount`, `period_months`, `annual_amount`, `calculation_type`, `benefit_evidence_text` 반환
- 포트폴리오 계산도 같은 추출기를 사용해 LLM 숫자 환각 가능성을 낮춤
