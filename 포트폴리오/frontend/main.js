...
async function loadPortfolio() {
  try {
    ...
    if (useBackend) {
      ...
      if (data && (data.portfolio_items || data.portfolio_hero)) {
        _renderPortfolioData(data);
        return;
      }
    }
  } catch (e) {
    console.warn('portfolio API fallback:', e);
  }
  _renderPortfolioStatic();
}

function _renderPortfolioData(data) {
  const isNewApi = Array.isArray(data.portfolio_items) && !data.portfolio_hero;
  const hero = data.portfolio_hero || {
    total_expected_benefit_label: data.total_estimated_benefit_label || '-',
    portfolio_basis_label: `탐욕 탐색 기반 최적 조합 ${data.selected_policy_count || 0}개 정책 기준`,
    ready_count: data.applicable_now_count || 0,
    conditional_count: data.needs_check_count || 0,
  };
  ...
  if (isNewApi && (data.calculation_basis_note || data.reasoning_summary)) {
    let basis = document.querySelector('.portfolio-basis');
    if (!basis) {
      basis = document.createElement('p');
      basis.className = 'portfolio-basis';
      document.querySelector('.portfolio-hero > div')?.appendChild(basis);
    }
    basis.textContent = [data.calculation_basis_note, data.reasoning_summary].filter(Boolean).join(' ');
  }
  ...
  _renderPortfolioMetrics(data, hero);
  _renderPortfolioBreakdown(data.category_breakdown || [], data.excluded_policy_count || 0);
  _renderPortfolioPipeline(data.pipeline_steps || []);
  _renderPortfolioExcluded(data.excluded_items || []);
  const sectionTitle = document.querySelector('#screen-portfolio .section-title h3');
  if (sectionTitle) sectionTitle.textContent = '추천 연간 수혜 조합';

  // port-grid 카드 렌더링
  const portGrid = document.querySelector('.port-grid');
  if (portGrid) {
    const items = (data.portfolio_items || []).map(item => _normalizePortfolioItem(item));
    const cards = items.map(item => `
      <article class="annual-policy-card">
        <div class="annual-card-main" onclick="showDetail('${escHtml(item.policy_id)}')" role="button" tabindex="0">
          <div class="annual-card-icon">${escHtml(item.icon)}</div>
          <div class="annual-card-copy">
            <div class="annual-card-title-row">
              <h4>${escHtml(item.title)}</h4>
              ${_portfolioStatusBadge(item.status)}
            </div>
            <p>${escHtml(item.selected_reason || item.fit_reasons[0] || '사용자 조건 기준 포트폴리오 후보입니다.')}</p>
            <div class="portfolio-meta">
              ${item.category ? `<span>${escHtml(item.category)}</span>` : ''}
              ${item.confidence ? `<span>산정 ${escHtml(item.confidence)}</span>` : ''}
              ${item.calculation_type ? `<span>${escHtml(_portfolioCalcTypeLabel(item.calculation_type))}</span>` : ''}
              ${item.guide_tags.slice(0, 2).map(tag => `<span>${escHtml(tag)}</span>`).join('')}
            </div>
          </div>
          <div class="annual-card-amount">
            <strong>${escHtml(item.amount_label)}</strong>
            <span>${item.is_cash_equivalent === false ? '총액 제외' : '연간 반영'}</span>
          </div>
        </div>
        <div class="annual-card-detail">
          <div class="annual-detail-block">
            <span>산정 근거</span>
            <strong>${escHtml(item.amount_basis || item.period_label || '공식 공고 확인')}</strong>
          </div>
          <div class="annual-detail-block">
            <span>원문 근거</span>
            <strong>${escHtml(item.evidence_quality || 'LOW')} · ${Number(item.grounded_score || 0)}점</strong>
          </div>
          <div class="annual-detail-block">
            <span>합산 여부</span>
            <strong class="${item.is_cash_equivalent === false ? 'portfolio-calc-warn' : 'portfolio-calc-ok'}">${item.is_cash_equivalent === false ? '제외' : '반영'}</strong>
          </div>
        </div>
        <div class="annual-card-support">
          ${item.fit_reasons.length ? `<div><b>조건 적합</b>${item.fit_reasons.slice(0, 2).map(reason => `<span>${escHtml(reason)}</span>`).join('')}</div>` : ''}
          ${item.required_actions.length ? `<div><b>다음 행동</b>${item.required_actions.slice(0, 2).map(text => `<span>${escHtml(text)}</span>`).join('')}</div>` : ''}
          ${item.risk_flags.length ? `<div class="annual-warning"><b>확인 필요</b><span>${escHtml(item.risk_flags[0])}</span></div>` : ''}
        </div>
        ${item.evidence_snippets.length ? `<div class="portfolio-evidence compact"><strong>원문 근거</strong>${item.evidence_snippets.slice(0, 1).map(text => `<span>${escHtml(text)}</span>`).join('')}</div>` : ''}
        ${item.overlap_note ? `<p class="portfolio-note">${escHtml(item.overlap_note)}</p>` : ''}
      </article>`).join('');

    portGrid.innerHTML = cards + `
      <div class="annual-add-card" onclick="showTab('dashboard')">
        <strong>정책 더 살펴보기</strong>
        <span>분석 결과에서 추가 후보를 확인하고 포트폴리오를 다시 구성하세요.</span>
      </div>`;
  }
  ...
  if (ctaH3) ctaH3.textContent = data.portfolio_cta?.headline || '🚀 이 조합으로 지금 바로 신청하세요!';
  if (ctaP)  ctaP.textContent  = data.portfolio_cta?.description || `총 ${hero.total_expected_benefit_label} 혜택을 한 번에 챙길 수 있어요`;
}

function _portfolioStatusBadge(status) {
  return status === 'ready' || status === 'APPLICABLE_NOW'
    ? '<span class="badge badge-green">즉시 신청</span>'
    : status === 'conditional' || status === 'NEEDS_CHECK'
      ? '<span class="badge badge-blue">조건 확인 필요</span>'
      : '<span class="badge badge-orange">조건 부족</span>';
}

function _portfolioCalcTypeLabel(type) {
  const labels = {
    MONTHLY_FIXED: '월 지급 환산',
    ANNUAL_FIXED: '연 지급',
    SESSION_BASED: '회차 지원',
    ONE_TIME_MAX: '최대/일시 지원',
    ASSET_CONTRIBUTION_ESTIMATE: '정부기여금 추정',
    LOAN_LIMIT: '대출 한도',
    UNKNOWN: '금액 확인 필요',
  };
  return labels[type] || type || '금액 확인 필요';
}
...
function _normalizePortfolioItem(item) {
  const title = item.title || item.policy_name || item.서비스명 || '복지 정책';
  const score = Number(item.match_score ?? item.수급확률 ?? 70);
  const status = item.apply_status || item.status || (score >= 80 ? 'ready' : score >= 60 ? 'conditional' : 'blocked');
  return {
    policy_id: item.policy_id || _slugPolicyName(title),
    icon: item.icon || _inferPolicyIcon(`${title} ${item.category || item.benefit_summary || ''}`),
    title,
    amount_label: item.annual_amount_label || item.amount_label || item.expected_benefit_label || item.benefit_label || '-',
    period_label: item.amount_basis || item.period_label || item.benefit_period_label || item.subtitle || '-',
    amount_basis: item.amount_basis || item.period_label || null,
    calculation_type: item.calculation_type || null,
    is_cash_equivalent: item.is_cash_equivalent !== false,
    fit_reasons: _arrayOrEmpty(item.fit_reasons),
    risk_flags: _arrayOrEmpty(item.risk_flags),
    required_actions: _arrayOrEmpty(item.required_actions),
    evidence_snippets: _arrayOrEmpty(item.evidence_snippets),
    guide_tags: _arrayOrEmpty(item.guide_tags),
    evidence_quality: item.evidence_quality || null,
    grounded_score: item.grounded_score || 0,
    category: item.category || null,
    confidence: item.confidence || null,
    status,
    selected_reason: item.selected_reason || null,
    overlap_note: item.overlap_note || null,
  };
}

function _renderPortfolioMetrics(data, hero) {
  let wrap = document.querySelector('.portfolio-metrics');
  const heroEl = document.querySelector('.portfolio-hero');
  if (!wrap && heroEl) {
    wrap = document.createElement('div');
    wrap.className = 'portfolio-metrics';
    heroEl.insertAdjacentElement('afterend', wrap);
  }
  if (!wrap) return;

  const selectedCount = Number(data.selected_policy_count || 0);
  const readyCount = Number(hero.ready_count || data.applicable_now_count || 0);
  const conditionalCount = Number(hero.conditional_count || data.needs_check_count || 0);
  const excludedCount = Number(data.excluded_policy_count || 0);
  const currency = data.currency || 'KRW';

  wrap.innerHTML = `
    <div class="portfolio-metric primary">
      <span>현금성 예상 총액</span>
      <strong>${escHtml(hero.total_expected_benefit_label || data.total_estimated_benefit_label || '-')}</strong>
      <em>${escHtml(currency)} · 연간 기준</em>
    </div>
    ...
    <div class="portfolio-metric muted">
      <span>분리/제외</span>
      <strong>${excludedCount}건</strong>
      <em>중복·대출·조건 불일치</em>
    </div>
  `;
}

function _renderPortfolioBreakdown(breakdown, excludedCount) {
  let wrap = document.querySelector('.portfolio-breakdown');
  const sectionTitle = document.querySelector('#screen-portfolio .section-title');
  if (!wrap && sectionTitle) {
    wrap = document.createElement('div');
    wrap.className = 'portfolio-breakdown';
    sectionTitle.insertAdjacentElement('afterend', wrap);
  }
  if (!wrap) return;
  ...
  const cards = (breakdown || []).map(item => `
    <div class="portfolio-breakdown-item">
      ...
      <strong>${escHtml(item.annual_amount_label)}</strong>
      <em>${Number(item.count || 0)}건</em>
    </div>`).join('');
  const excluded = excludedCount
    ? `<div class="portfolio-breakdown-item portfolio-excluded"><span>중복/조건 제외</span><strong>${excludedCount}건</strong><em>자동 필터링</em></div>`
    : '';
  wrap.innerHTML = cards + excluded;
}

function _renderPortfolioExcluded(items) {
  let wrap = document.querySelector('.portfolio-excluded-panel');
  const breakdown = document.querySelector('.portfolio-breakdown');
  const sectionTitle = document.querySelector('#screen-portfolio .section-title');
  if (!wrap) {
    wrap = document.createElement('div');
    wrap.className = 'portfolio-excluded-panel';
    if (breakdown) breakdown.insertAdjacentElement('afterend', wrap);
    else sectionTitle?.insertAdjacentElement('afterend', wrap);
  }
  if (!wrap) return;

  const visible = _arrayOrEmpty(items).slice(0, 5);
  if (!visible.length) {
    wrap.innerHTML = '';
    return;
  }
  ...
  wrap.innerHTML = `
    <div class="portfolio-excluded-head">
      <strong>AI 제외/보류 판단</strong>
      <span>중복, 조건 불일치, 현금성 제외 항목</span>
    </div>
    ${visible.map(item => `
      <div class="portfolio-excluded-row">
        <div>
          <strong>${escHtml(item.title || '정책')}</strong>
          <span>${escHtml(item.reason || '조건 확인 필요')}</span>
          ${_arrayOrEmpty(item.evidence_snippets).length ? `<em>${escHtml(item.evidence_snippets[0])}</em>` : ''}
        </div>
        <b>${escHtml(item.annual_amount_label || '-')}</b>
      </div>
    `).join('')}
  `;
}

function _renderPortfolioPipeline(steps) {
  let wrap = document.querySelector('.portfolio-pipeline-panel');
  const breakdown = document.querySelector('.portfolio-breakdown');
  const sectionTitle = document.querySelector('#screen-portfolio .section-title');
  if (!wrap) {
    wrap = document.createElement('div');
    wrap.className = 'portfolio-pipeline-panel';
    if (breakdown) breakdown.insertAdjacentElement('afterend', wrap);
    else sectionTitle?.insertAdjacentElement('afterend', wrap);
  }
  if (!wrap) return;

  const visible = _arrayOrEmpty(steps).slice(0, 6);
  if (!visible.length) {
    wrap.innerHTML = '';
    return;
  }
  ...
  wrap.innerHTML = `
    <div class="portfolio-pipeline-head">
      <strong>연간 수혜액 포트폴리오 파이프라인</strong>
      <span>입력 → 정규화 → 필터링 → 조합 추천</span>
    </div>
    <div class="portfolio-pipeline-steps">
      ${visible.map(step => `
        <div class="portfolio-pipeline-step">
          <b>${Number(step.step_order || 0)}</b>
          <div>
            <strong>${escHtml(step.step_name || step.step_code || '단계')}</strong>
            <span>${escHtml(step.process || '')}</span>
            ${step.guardrail ? `<em>${escHtml(step.guardrail)}</em>` : ''}
          </div>
        </div>
      `).join('')}
    </div>
      ...
      const cards = _currentPortfolio.map(card => {
        ...
        return `
          <div class="port-grid-card" onclick="showDetail('${card.policy_id}')" style="cursor:pointer;">
            ...
            <div style="margin-top:10px;">${_portfolioStatusBadge(status)}</div>
          </div>`;
      }).join('');
      portGrid.innerHTML = cards + `
