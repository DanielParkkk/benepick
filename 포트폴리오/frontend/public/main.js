...
async function loadPortfolio() {
  try {
    const payload = await apiFetch('/portfolio');
    const data = payload?.data || payload;
    if (data && (Array.isArray(data.portfolio_items) || Array.isArray(data.items))) {
      _renderPortfolioData(data);
      return;
    }
  } catch (e) {
    console.warn('[portfolio] backend portfolio fetch failed:', e?.message || e);
  }
...
function _renderPortfolioData(data) {
  ...
  const items = Array.isArray(source.portfolio_items) ? source.portfolio_items : (source.items || []);
  const hero = source.portfolio_hero || {};
  const totalLabel = source.total_estimated_benefit_label || hero.total_expected_benefit_label || '-';
  const basisLabel = source.calculation_note || hero.portfolio_basis_label || `${items.length}개 추천 정책 기준 예상 수혜액입니다.`;
  const readyCount = source.applicable_now_count ?? hero.ready_count ?? 0;
  const conditionalCount = source.needs_check_count ?? hero.conditional_count ?? 0;

  // hero 총 수혜액
  const bigNum = document.querySelector('.portfolio-hero .big-num');
  if (bigNum) bigNum.textContent = totalLabel;

  // hero 설명 p 태그 (두 번째 p)
  const heroPs = document.querySelectorAll('.portfolio-hero p');
  if (heroPs.length >= 2) heroPs[1].textContent = basisLabel;
  else if (heroPs.length === 1) heroPs[0].textContent = basisLabel;

  // hero 배지: 즉시 신청 N건 / 조건 보완 후 M건
  const heroBadges = document.querySelectorAll('.portfolio-hero span');
  if (heroBadges.length >= 2) {
    heroBadges[0].textContent = `✅ 즉시 신청 가능 ${readyCount}건`;
    heroBadges[1].textContent = `⚡ 조건 확인 필요 ${conditionalCount}건`;
  }
  ...
  if (portGrid) {
    const cards = items.map(item => {
      const status = _normalizePortfolioStatus(item.apply_status || item.status);
      const policyId = _escapeOnclickArg(item.policy_id || item.id || '');
      const title = item.title || item.policy_name || item.서비스명 || '정책명 확인 필요';
      const amount = item.amount_label || item.expected_benefit_label || item.benefit_label || '금액 확인 필요';
      const period = item.period_label || item.benefit_period_label || item.calculation_basis || '지원 기간 확인 필요';
      return `
      <div class="port-grid-card" onclick="showDetail('${policyId}')" style="cursor:pointer;">
        <div class="icon">${escHtml(item.icon || _portfolioIconForItem(item))}</div>
        <h4>${escHtml(title)}</h4>
        <div class="amount">${escHtml(amount)}</div>
        <div class="period">${escHtml(period)}</div>
  ...
  const cta = source.portfolio_cta || {};
  if (ctaH3) ctaH3.textContent = cta.headline || '나에게 맞는 복지 포트폴리오를 계속 관리하세요';
  if (ctaP)  ctaP.textContent  = cta.description || '추천 정책의 예상 수혜액과 신청 상태를 한 화면에서 확인할 수 있습니다.';
}

function _normalizePortfolioStatus(status) {
  if (status === 'APPLICABLE_NOW' || status === 'ready') return 'ready';
  if (status === 'NEEDS_CHECK' || status === 'conditional') return 'conditional';
  return 'blocked';
}

function _portfolioStatusBadge(status) {
  return status === 'ready'       ? '<span class="badge badge-green">즉시 신청</span>' :
         status === 'conditional' ? '<span class="badge badge-blue">조건 확인 필요</span>' :
                                    '<span class="badge badge-orange">조건 부족</span>';
}

function _portfolioIconForItem(item) {
  const text = `${item.title || item.policy_name || ''} ${item.source || ''} ${item.managing_agency || ''}`;
  if (text.includes('월세') || text.includes('주거')) return '🏠';
  if (text.includes('교육') || text.includes('훈련') || text.includes('내일배움')) return '🎓';
  if (text.includes('취업') || text.includes('일자리') || text.includes('창업')) return '💼';
  if (text.includes('아동') || text.includes('가족') || text.includes('부모')) return '👪';
  if (text.includes('의료') || text.includes('건강')) return '🏥';
  return '📋';
}
