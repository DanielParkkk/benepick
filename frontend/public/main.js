...
Object.defineProperty(window, '_currentPortfolio', { get: () => _currentPortfolio, set: v => { _currentPortfolio = v; } });
...
function _getUserId() {
  try {
    ...
    return payload.user_id || payload.sub || payload.id || 'guest';
  } catch(e) { return 'guest'; }
}
const SCRAP_KEY = () => `benefic_scraps_${_getUserId()}`;
const RV_KEY    = () => `benefic_rv_${_getUserId()}`;
...
function toggleScrap(policyId, btnEl) {
  ...
  const willScrap = !btnEl.classList.contains('active');
  ...
  // API 저장
  fetch('/api/scraps', {
    method: willScrap ? 'POST' : 'DELETE',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    body: JSON.stringify({ policy_id: policyId }),
  }).catch(e => console.warn('스크랩 저장 실패:', e));
}
...
// ── 폼 값 → 한국어 정규화 (전 언어 대응) ────────────────────
function _normalizeToKorean(field, value) {
  ...
  const maps = {
    ...
    family: {
      // 영어
      '1-person':'1인 가구', '2-person':'2인 가구', '3-person':'3인 가구',
      '4+ persons':'4인 이상 가구', 'single-parent':'한부모 가구', 'multi-child':'다자녀 가구',
      'multicultural':'다문화 가구', 'grandparent-grandchild':'조손 가구', 'elderly solo':'노인 단독 가구',
      // 중국어 (간체)
      '1人家庭':'1인 가구', '2人家庭':'2인 가구', '3人家庭':'3인 가구',
      '4人以上家庭':'4인 이상 가구', '单亲家庭':'한부모 가구', '多子女家庭':'다자녀 가구',
      '多文化家庭':'다문화 가구', '祖孙家庭':'조손 가구', '独居老人':'노인 단독 가구',
      // 일본어
      '1人世帯':'1인 가구', '2人世帯':'2인 가구', '3人世帯':'3인 가구',
      '4人以上世帯':'4인 이상 가구', 'ひとり親世帯':'한부모 가구', '多子世帯':'다자녀 가구',
      '多文化世帯':'다문화 가구', '祖父母+孫世帯':'조손 가구', '高齢者単独':'노인 단독 가구',
      // 베트남어
      '1 người':'1인 가구', '2 người':'2인 가구', '3 người':'3인 가구',
      '4 người trở lên':'4인 이상 가구', 'gia đình đơn thân':'한부모 가구',
      'gia đình nhiều con':'다자녀 가구', 'gia đình đa văn hóa':'다문화 가구',
      'ông bà - cháu':'조손 가구', 'người cao tuổi sống một mình':'노인 단독 가구',
    },

    // ── 취업 상태 ──────────────────────────────────────────────
    employment: {
      // 영어
      'unemployed':'미취업', 'employed (full-time)':'취업자 (정규직)',
      'employed (part-time/contract)':'취업자 (비정규직/계약직)',
      'self-employed':'자영업자', 'job-seeker':'구직자 (실업)',
      'student':'학생', 'parental leave':'육아휴직 중', 'not working':'무직',
      // 중국어
      '未就业':'미취업', '就业（正式）':'취업자 (정규직)',
      '就业（非正式/合同）':'취업자 (비정규직/계약직)',
      '自营业者':'자영업자', '求职者（失业）':'구직자 (실업)',
      '学生':'학생', '育儿假中':'육아휴직 중', '无业':'무직',
      // 일본어
      '未就業':'미취업', '就業中（正社員）':'취업자 (정규직)',
      '就業中（非正規/契約）':'취업자 (비정규직/계약직)',
      '自営業者':'자영업자', '求職中（失業）':'구직자 (실업)',
      '学生':'학생', '育児休業中':'육아휴직 중', '無職':'무직',
      // 베트남어
      'chưa có việc làm':'미취업', 'có việc làm (chính thức)':'취업자 (정규직)',
      'có việc làm (bán thời gian/hợp đồng)':'취업자 (비정규직/계약직)',
      'tự kinh doanh':'자영업자', 'đang tìm việc (thất nghiệp)':'구직자 (실업)',
      'học sinh/sinh viên':'학생', 'nghỉ thai sản':'육아휴직 중', 'không làm việc':'무직',
    },
    ...
    disability: {
      // 영어
      'none':'없음',
      'disability lv.1~3 (severe)':'장애 1~3급 (심한 장애)',
      'disability lv.4~6 (mild)':'장애 4~6급 (심하지 않은 장애)',
      // 중국어
      '无':'없음', '残疾1~3级（严重）':'장애 1~3급 (심한 장애)', '残疾4~6级（轻度）':'장애 4~6급 (심하지 않은 장애)',
      // 일본어
      'なし':'없음', '障害1~3級（重度）':'장애 1~3급 (심한 장애)', '障害4~6級（軽度）':'장애 4~6급 (심하지 않은 장애)',
      // 베트남어
      'không':'없음', 'khuyết tật mức 1~3 (nặng)':'장애 1~3급 (심한 장애)', 'khuyết tật mức 4~6 (nhẹ)':'장애 4~6급 (심하지 않은 장애)',
    },
    ...
    region: {
      ...
      // 중국어
      '首尔特别市':'서울특별시', '釜山广域市':'부산광역시', '大邱广域市':'대구광역시',
      '仁川广域市':'인천광역시', '光州广域市':'광주광역시', '大田广域市':'대전광역시',
      '蔚山广域市':'울산광역시', '世宗特别自治市':'세종특별자치시', '京畿道':'경기도',
      '江原特别自治道':'강원특별자치도', '忠清北道':'충청북도', '忠清南道':'충청남도',
      '全北特别自治道':'전북특별자치도', '全罗南道':'전라남도', '庆尚北道':'경상북도',
      '庆尚南道':'경상남도', '济州特别自治道':'제주특별자치도',
      // 일본어
      'ソウル特別市':'서울특별시', '釜山広域市':'부산광역시', '大邱広域市':'대구광역시',
      '仁川広域市':'인천광역시', '光州広域市':'광주광역시', '大田広域市':'대전광역시',
      '蔚山広域市':'울산광역시', '世宗特別自治市':'세종특별자치시', '京畿道':'경기도',
      '江原特別自治道':'강원특별자치도', '忠清北道':'충청북도', '忠清南道':'충청남도',
      '全北特別自治道':'전북특별자치도', '全羅南道':'전라남도', '慶尚北道':'경상북도',
      '慶尚南道':'경상남도', '済州特別自治道':'제주특별자치도',
    },

    // ── 소득 수준 ──────────────────────────────────────────────
    income: {
      // 영어
      'below 30% median':'중위소득 30% 이하', '30~40% median':'중위소득 30~40%',
      '40~50% median':'중위소득 40~50%', '50~60% median':'중위소득 50~60%',
      '60~70% median':'중위소득 60~70%', '70~80% median':'중위소득 70~80%',
      '80~90% median':'중위소득 80~90%', '90~100% median':'중위소득 90~100%',
      '100~120% median':'중위소득 100~120%', '120~150% median':'중위소득 120~150%',
      'above 150% median':'중위소득 150% 초과',
      // 중국어
      '中位收入30%以下':'중위소득 30% 이하', '中位收入30~40%':'중위소득 30~40%',
      '中位收入40~50%':'중위소득 40~50%', '中位收入50~60%':'중위소득 50~60%',
      '中位收入60~70%':'중위소득 60~70%', '中位收入70~80%':'중위소득 70~80%',
      '中位收入80~90%':'중위소득 80~90%', '中位收入90~100%':'중위소득 90~100%',
      '中位收入100~120%':'중위소득 100~120%', '中位收入120~150%':'중위소득 120~150%',
      '超过中位收入150%':'중위소득 150% 초과',
      // 일본어
      '中位所得30%以下':'중위소득 30% 이하', '中位所得30~40%':'중위소득 30~40%',
      '中位所得40~50%':'중위소득 40~50%', '中位所得50~60%':'중위소득 50~60%',
      '中位所得60~70%':'중위소득 60~70%', '中位所得70~80%':'중위소득 70~80%',
      '中位所得80~90%':'중위소득 80~90%', '中位所得90~100%':'중위소득 90~100%',
      '中位所得100~120%':'중위소득 100~120%', '中位所得120~150%':'중위소득 120~150%',
      '中位所得150%超過':'중위소득 150% 초과',
      // 베트남어
      'dưới 30% thu nhập trung vị':'중위소득 30% 이하', '30~40% trung vị':'중위소득 30~40%',
      '40~50% trung vị':'중위소득 40~50%', '50~60% trung vị':'중위소득 50~60%',
      '60~70% trung vị':'중위소득 60~70%', '70~80% trung vị':'중위소득 70~80%',
      '80~90% trung vị':'중위소득 80~90%', '90~100% trung vị':'중위소득 90~100%',
      '100~120% trung vị':'중위소득 100~120%', '120~150% trung vị':'중위소득 120~150%',
      'trên 150% trung vị':'중위소득 150% 초과',
    },
  };

  const map = maps[field];
  if (!map) return v;
  // 대소문자 무관 매칭 (영어/베트남어 대응)
...
function collectFormData() {
  ...
  const regionVal     = _normalizeToKorean('region',     document.getElementById('sel-region')?.value     || '서울특별시');
  const incomeVal     = _normalizeToKorean('income',     document.getElementById('sel-income')?.value     || '중위소득 50~60%');
  const familyVal     = _normalizeToKorean('family',     document.getElementById('sel-family')?.value     || '1인 가구');
  const empVal        = _normalizeToKorean('employment', document.getElementById('sel-employment')?.value || '미취업');
  const disabilityVal = _normalizeToKorean('disability', document.getElementById('sel-disability')?.value || '없음');
...
async function showDetail(policyId) {
  ...
  if (token) {
    fetch('/api/recently-viewed', {
      ...
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ policy_id: policyId }),
    }).catch(e => console.warn('최근 본 공고 저장 실패:', e));
  }
  ...
  // analysis 페이지 안에서 직접 호출된 경우: 이동 없이 바로 렌더링
  // sessionStorage에만 저장 (뒤로가기 복원용, history 스택은 건드리지 않음)
  try { sessionStorage.setItem('benefic_current_detail', policyId); } catch(e) {}
...
function goToPolicyDetailPage(policyKey) {
  ...
  if (token) {
    fetch('/api/recently-viewed', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ policy_id: policyKey }),
    }).catch(e => console.warn('최근 본 공고 저장 실패:', e));
  }
...
// 카드 렌더 후 API로 스크랩 상태 동기화
async function _syncScrapButtons() {
  const token = localStorage.getItem('token');
  if (!token) return;
  try {
    const res = await fetch('/api/scraps', { headers: { Authorization: `Bearer ${token}` } });
    if (!res.ok) return;
    const json = await res.json();
    const scrappedIds = new Set((json.data || []).map(r => r.policy_id));
    document.querySelectorAll('.scrap-btn[data-policy-id]').forEach(btn => {
      const pid = btn.getAttribute('data-policy-id');
      const active = scrappedIds.has(pid);
      btn.classList.toggle('active', active);
      btn.textContent = active ? '★' : '☆';
      btn.title = active ? '스크랩 해제' : '스크랩 저장';
      btn.setAttribute('aria-label', active ? '스크랩 해제' : '스크랩 저장');
    });
  } catch(e) {}
}
...
function showCommDetail(id) {
  ...
  document.getElementById('commDetailCard').innerHTML = `
    ...
    ${(() => {
      const u = getAuthUser();
      const isOwner = u && u.uid && post.author_uid ? u.uid === post.author_uid : false;
      return (isOwner || _isAdmin()) ? `<button onclick="openDeleteModal(${id})" style="margin-left:8px;background:#fff0f0;border:1px solid #e53e3e;border-radius:8px;padding:8px 16px;font-size:13px;font-weight:600;color:#e53e3e;cursor:pointer;">🗑️ 삭제</button>` : '';
    })()}
  `;
...
function _isAdmin() {
  try { return !!localStorage.getItem('benefic_admin_token'); } catch(e) { return false; }
}
...
function closeDeleteModal(e) {
  if (!e || e.target === document.getElementById('deleteModalBg')) {
...
function confirmDelete() {
  ...
  const post = commPosts.find(p => p.id === _deleteTargetId);
  if (!post) { closeDeleteModalDirect(); return; }
  ...
  commPosts = commPosts.filter(p => p.id !== _deleteTargetId);
  savePosts();
  closeDeleteModalDirect();
  showCommList();
}
...
window.addEventListener('load', () => {
  ...
  if (currentPage === '/analysis') {
    ...
    if (pid) {
      ...
      // sessionStorage 복원 (같은 탭 내 뒤로가기)
      const pidSession = (() => { try { return sessionStorage.getItem('benefic_current_detail'); } catch(e) { return null; } })();
      if (pidSession) showDetail(pidSession);
    }
  }
});
