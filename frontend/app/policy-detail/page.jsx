...
export default function PolicyDetailPage() {
  useEffect(() => {
    ...
    window.toggleScrap = async function(policyId) {
      ...
      } catch(e) { console.error('[스크랩 오류]', e); }
    };
    ...
    function renderPolicyDetail(p, related, policyId) {
      p._policyId = policyId;
      ...
      document.getElementById('pd-content').innerHTML = `
        <div class="pd-hero">
          ...
          <div class="pd-hero-actions">
            <button class="pd-scrap-btn" id="pdScrapBtn" onclick="toggleScrap('${policyId}')">☆ 스크랩</button>
      ...
      // 스크랩 여부 서버에서 확인
      try {
        const res = await fetch('/api/scraps', { headers: { Authorization: `Bearer ${getAuthToken()}` } });
        if (res.ok) {
          const json = await res.json();
          const ids = (json.data || []).map(r => r.policy_id);
          if (ids.includes(p._policyId)) {
            _isScraped = true;
            const btn = document.getElementById('pdScrapBtn');
    ...
    async function saveRecentlyViewed(policyId) {
      try {
      ...
      } catch(e) { console.error('[최근 본 공고 저장 오류]', e); }
    }
    ...
    async function initPolicyDetail() {
      ...
      if (policy) {
        ...
        renderPolicyDetail(policy, related, policyKey);
        saveRecentlyViewed(policyKey);
        return;
      }
      ...
      if (isNumericId) {
        try {
          ...
          if (res.ok) {
            ...
            renderPolicyDetail(backendPolicy, [], policyKey);
            saveRecentlyViewed(policyKey);
            return;
          }
        } catch(e) {}
